...
export const tools: Tool[] = [
  { number: 1, slug: 'gate-mcp', name: 'Kineti Gate for MCP', detail: 'Local proxy: allow, ask, or deny. Approval binds to the exact action.', docs: 'A local Rust proxy that sits in front of MCP tool calls. Set allow, ask, or deny policies for tool and argument patterns. Human approval binds to the exact action hash: change one argument and the approval voids. Hard per-run and per-project spend stops. Every call writes a dual-signed evidence receipt.', who: 'Teams whose agents touch code, production, or customer data.', status: 'In sprint', tier: 'The wedge', date: 'Sep 21-Nov 1' },
  { number: 2, slug: 'verifier-spec', name: 'Kineti Verifier + Spec', detail: 'Check any receipt without trusting Kineti cloud.', docs: 'An independent CLI plus the public receipt and policy spec. Anyone can verify a Kineti receipt without running Kineti or trusting its servers. The spec, the verifier, and the gate are all MIT.', who: 'Buyers, auditors, skeptics.', status: 'Week 3', tier: 'The wedge', date: 'Oct 5-11' },
  { number: 3, slug: 'kineti-talk', name: 'kineti_talk', detail: 'One MCP tool across 10 editors and hosts.', docs: 'One MCP tool that puts Kineti inside 10 editors and agent hosts. Talk normal, no commands. Shipped and working in v0.3.2.', who: 'Solo builders running agents in their editor.', status: 'Shipped in v0.3.2', tier: 'Built, ready to extract', date: 'Extractable' },
  { number: 4, slug: 'spend-stop', name: 'Kineti Spend Stop', detail: 'Hard spend ceiling with human-only reset.', docs: 'A hard spending ceiling for any agent setup. Per-agent splits, per-run and per-project stops. Only a human resets it. For anyone burned by a runaway agent bill.', who: 'Anyone paying model bills.', status: 'Extractable', tier: 'Built, ready to extract', date: 'Extractable' },
  { number: 5, slug: 'undo', name: 'Kineti Undo', detail: 'Every change registers its inverse. Undo needs your yes.', docs: 'Undo for agent actions. Every change registers its inverse before it runs. Nothing reverses without your approval. Where an action has no inverse, Kineti says so up front.', who: 'Teams letting agents change real systems.', status: 'Extractable', tier: 'Built, ready to extract', date: 'Extractable' },
  { number: 6, slug: 'memory', name: 'Kineti Memory', detail: 'Project-scoped memory. Forgets with a proof receipt.', docs: 'Memory that cannot leak across projects. Scope isolation per project, certainty tiers per fact, tombstones on delete. Forgetting writes a proof receipt.', who: 'Teams running agents across many projects.', status: 'Extractable', tier: 'Built, ready to extract', date: 'Extractable' },
  { number: 7, slug: 'ovt', name: 'Kineti OVT', detail: 'Scores dollars per true outcome.', docs: 'Proof of what your agent actually achieved. Outcome verification plus dollars-per-true-outcome scoring. Measures results, not model vibes.', who: 'Teams justifying agent spend.', status: 'Extractable', tier: 'Built, ready to extract', date: 'Extractable' },
  { number: 8, slug: 'dashboard', name: 'Kineti Dashboard', detail: 'Token-locked local view: goals, spend, undo, proof.', docs: 'The local dashboard: goal, spend, undo, proof, on/off switch, one talk box. Token locked on your machine. Encrypted sync between machines.', who: 'Operators watching agents daily.', status: 'Extractable', tier: 'Built, ready to extract', date: 'Extractable' },
  { number: 9, slug: 'policy-packs', name: 'Kineti Policy Packs', detail: 'Starter rules for code, money, data, production.', docs: 'Starter policy templates for the gate. Code changes, money movement, customer data, production operations. Pick one, tighten it, ship.', who: 'New gate users.', status: 'Small new build', tier: 'Next small builds', date: 'Next' },
  { number: 10, slug: 'hostile-100', name: 'Kineti Hostile 100', detail: '100 hostile tool calls. Method and failures public.', docs: 'A public benchmark of 100 hostile tool calls against the gate. Method published, every failure published. The bar the gate must clear.', who: 'Skeptics and contributors.', status: 'Test suite', tier: 'Next small builds', date: 'Next' },
  { number: 11, slug: 'receipt-viewer', name: 'Kineti Receipt Viewer', detail: 'Renders a signed receipt readable.', docs: 'Turns signed receipt JSON into a readable record: action, policy, approver, hash, outcome. For humans checking what happened.', who: 'Auditors and operators.', status: 'Small new build', tier: 'Next small builds', date: 'Next' },
  { number: 12, slug: 'demo', name: 'Kineti Demo', detail: 'Approve $10, try $100, watch it block.', docs: 'The 90-second dangerous-action demo as a runnable script. Approval set for $10, model changes it to $100, Kineti blocks it and writes a verifiable receipt.', who: 'Evaluators with ten minutes.', status: 'Small new build', tier: 'Next small builds', date: 'Next' },
  { number: 13, slug: 'fleet', name: 'Kineti Fleet', detail: 'One view across many projects.', docs: 'Many projects, one view. Postponed until the wedge proves out.', who: 'Teams running Kineti widely.', status: 'Later', tier: 'After the wedge proves out', date: 'Later' },
  { number: 14, slug: 'cloud-link', name: 'Kineti Cloud Link', detail: 'Browser view via one-use codes and encrypted store.', docs: 'The cloud dashboard link. One-use pairing codes, GitHub login, encrypted store. Postponed until the wedge proves out.', who: 'Teams wanting a browser view.', status: 'Later', tier: 'After the wedge proves out', date: 'Later' },
  { number: 15, slug: 'activity-feed', name: 'Kineti Activity Feed', detail: 'Embeddable audit trail.', docs: 'The view-only audit, proof, and spend trail as an embeddable component. Postponed until the wedge proves out.', who: 'Teams embedding proof in their own dashboards.', status: 'Later', tier: 'After the wedge proves out', date: 'Later' },
];

export const tiers = [
  'The wedge',
  'Built, ready to extract',
  'Next small builds',
  'After the wedge proves out',
];
...
export const weeks: Array<[string, string, string]> = [
  ['Sep 21-27', 'Proxy foundation', 'One real MCP tool call passes through.'],
  ['Sep 28-Oct 4', 'Exact-action policy', 'A changed argument voids approval.'],
  ['Oct 5-11', 'Spend stops + verifier', 'A third party verifies a receipt.'],
  ['Oct 12-18', 'Reference workflow + undo', 'Ten scripted failures recover.'],
  ['Oct 19-25', 'Docs, demo + this site', 'Five developers install alone.'],
  ['Oct 26-Nov 1', 'Design-partner use', 'Three teams run a real workflow.'],
];
