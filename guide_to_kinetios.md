...
### Dynamic Bypass Router

Kineti OS enforces a strict sequential path, but dynamically adapts to Persona capabilities:

**Rule:** If Tier 3 (SDLC Pipeline) or Tier 4 (Enterprise & Design) is inactive in `kineti.config.json` (e.g., for Persona D or E), the OS automatically suppresses UI-centric commands like `/design` and `/autoplan`. The system will route the user directly from `/spec` to specialized execution (e.g., `/train`, `/eval`, or direct backend implementation) to prevent cyclic LLM hallucinations.
